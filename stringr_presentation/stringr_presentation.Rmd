...
matrix with a column for each () group in pattern.  (Also **`str_match_all()`**)
```{r}
...
```
***
These stringr functions are locale-sensitive, which means that they can perform differently to accommodate different languages.  
...
**stringr** recreates essential base R functions with simpler, more consistent syntax.  
  
**stringr** functions can be used with the pipe operator.
```{r}
letters %>% str_c(collapse = "") %>% str_dup(3)
```
